...
| D3 | 范围 | 页面 7 个：首页 + 文章 + 说说 + 相册 + 关于 + 书架(P1) + 杂谈。**删除工具箱、小游戏** |
...
- **T-W1 中间层骨架 ✅**：`worker-bff/`（Hono on Workers）已建，本地验证通过 —— `/health` ok、`/api/*` 代理回源并命中缓存（`x-cache: HIT`，`s-maxage=60, swr=300`）、`/bff/home` 聚合正常、失效接口带 HMAC。待 CF Token 后部署
- **T2 / T3 ✅**：Astro 7 骨架 + M3 设计系统已上线 `https://neutronstar-web.pages.dev`（/、/about）
...
- 下一步：**T-W2 部署中间层**（前端 base 从直连后端切到同源 `/api`，拿边缘缓存）→ T5 剩余页（`/archive`、`/moments`、`/albums` 真实数据；动画留 T6/T7）
...
3. **按需加载**：`Lightbox`、`motion` 重组件、音乐播放器、Live2D 全部 `dynamic import`
...
| 主机 | 指向 |
|:--|:--|
| `neutronstar.fun` / `www` | CF Pages（Astro 主站） |
| `kirameku-api.neutronstar.fun` | **中间层 Workers**（对外 API 入口）→ 未命中回源 NAS FastAPI |
| 源站 NAS | 仅被中间层访问（隧道），不直接对外（写操作除外） |

> 现有 `kirameku-api.neutronstar.fun` 已指向 NAS FastAPI；加中间层后把该主机名改挂到 Workers，源站改用隧道子域（如 `origin.neutronstar.fun`）。
...
| **T-W1** | 中间层骨架：Hono Worker（代理 + Cache API + 图片 + HMAC 失效） | ✅ 本地验证通过，待部署 |
| **T-W2** | 聚合接口 `/bff/*` + KV tag 索引 + `/internal/revalidate` | 待做 |
| **T-W3** | `/img/*` 图片优化 + 长缓存；部署到 Workers + 域名切换 | 待做（需 CF Token） |
