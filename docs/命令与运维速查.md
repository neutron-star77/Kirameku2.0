...
> 版本自查（2026-09-15 起）：`curl -s -o NUL -D - https://neutronstar.fun/ | grep -i x-build-id` 输出的 7 位 SHA 与 web 仓 `git rev-parse --short HEAD` 一致即线上最新（HTML 层 `no-store` 永不陈旧，一致=已部署；不一致=CI 未跑完）。⚠️ **必须用 GET（`-o NUL -D -`）不要用 `curl -sI`（HEAD）**——middleware 对非 GET 请求直接放行，HEAD 拿不到 `X-Build-Id`，会误判未上线。⚠️ CI 部署约 2-3 分钟延迟：推送后前几次 GET 可能仍是旧版 worker（有 `X-HTML-Cache` 但无或旧 `X-Build-Id`），等约 2 分钟再查。浏览器仍显示旧功能 → `Ctrl+Shift+R` 或**无痕窗口**复核（休眠标签恢复不算刷新，不发新请求）。
